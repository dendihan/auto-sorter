...
uses UnDM, UnMain, Unparaminedit,Unparamoutedit;
...
//    DM.ADO_URGP_query.Edit;
    ...
    if save=1 then
      FormMain.RecordLog('添加入口:'+DM.ADO_PAIN_query.FieldByName('CODE00').AsString);
...
//    DM.ADO_URGP_query.Edit;
    //freeandnil(Formshowcol);
    if save=1 then
      FormMain.RecordLog('添加出口:'+DM.ADO_POUT_query.FieldByName('CODE00').AsString);
  ...
  if DM.ADO_PAIN_query.RecordCount>0 then
    begin
      if Application.MessageBox('确定删除选定入口？ ','确认',MB_YESNO) = 6 then
        begin
        Formmain.RecordLog('删除入口:'+DM.ADO_PAIN_query.FieldByName('CODE00').AsString);
...
//    DM.ADO_URGP_query.Edit;
    //freeandnil(Formshowcol);
    if save=1 then
      FormMain.RecordLog('编辑入口:'+DM.ADO_PAIN_query.FieldByName('CODE00').AsString);
    Formparaminedit.Free;
    Formparaminedit:=nil;
END;
end;
 ...
 if DM.ADO_POUT_query.RecordCount>0 then
    begin
      if Application.MessageBox('确定删除选定出口？ ','确认',MB_YESNO) = 6 then
        begin
        Formmain.RecordLog('删除出口:'+DM.ADO_POUT_query.FieldByName('CODE00').AsString);
...
//    DM.ADO_URGP_query.Edit;
    //freeandnil(Formshowcol);
    if save=1 then
      FormMain.RecordLog('编辑出口:'+DM.ADO_POUT_query.FieldByName('CODE00').AsString);
    Formparamoutedit.Free;
    Formparamoutedit:=nil;
END;
end;
