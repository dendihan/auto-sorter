...
//    DM.ADO_URGP_query.Edit;
    ...
    if save=1 then
      FormMain.RecordLog('添加用户:'+DM.ADO_USER_query.FieldByName('NAME00').AsString);
    Formuseredit.Free;
    Formuseredit:=nil;
end;
  ...
  if DM.ADO_USER_query.RecordCount>0 then
    begin
      if Application.MessageBox('确定删除选定用户？ ','确认',MB_YESNO) = 6 then
        begin
        Formmain.RecordLog('删除用户:'+DM.ADO_USER_query.FieldByName('NAME00').AsString);
...
//    DM.ADO_URGP_query.Edit;
    //freeandnil(Formshowcol);
    if save=1 then
      FormMain.RecordLog('编辑用户:'+DM.ADO_USER_query.FieldByName('NAME00').AsString);
    Formuseredit.Free;
    Formuseredit:=nil;
END;
end;
