...
//    DM.ADO_URGP_query.Edit;
    ...
    if save=1 then
      FormMain.RecordLog('添加角色:'+DM.ADO_URGP_query.FieldByName('NAME00').AsString);
    Formroleedit.Free;
    Formroleedit:=nil;
end;
  ...
  if DM.ADO_URGP_query.RecordCount>0 then
    begin
      ...
      //---------should delete all of action
        ...
        Formmain.RecordLog('删除角色:'+DM.ADO_URGP_query.FieldByName('NAME00').AsString);
...
//    DM.ADO_URGP_query.Edit;
    //freeandnil(Formshowcol);
    if save=1 then
      FormMain.RecordLog('编辑角色:'+DM.ADO_URGP_query.FieldByName('NAME00').AsString);
    Formroleedit.Free;
    Formroleedit:=nil;
END;
end;
