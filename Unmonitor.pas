...
uses
  ...
  Vcl.OleServer, OPCAutomation_TLB,UnRecContextData,Data.DB, IdScheduler,
...
//      DM.ADOQuery.ExecSQL;
    ...
    on e: Exception do
      formmain.ErrorLog('UpdateDBByBarCode 异常：' + sqlstr + '。 ' + e.Message);
  end;
end;
...
begin
  ...
  begin
    try
      ...
      on e: Exception do
        formmain.ErrorLog('WriteOuterForMain 异常：' + GlobalGuid + ' -> ' + GlobalOuter + ' . ' + e.Message);
    end;
  end;
end;
...
//        showmessage('1');
   ...
   //       p2 := Scanread.IndexOf('#');
        ...
        // 可能出现多个条码 条码1/条码2/条码3...
                ...
                on e: Exception do
                  formmain.ErrorLog('控制器数据解析：' + msg + ' -> ' + e.Message);
              end;
      // UpdateDBByBarCode;
            end;
        end;
        ...
        on e: exception do
          Formmain.ErrorLog('监控接收数据处理异常： ' + e.Message);
        end;
    end;
end;
...
//      outer := DM.ADOQuery.FieldByName('POUT').AsString;
    ...
    on e: Exception do
      formmain.ErrorLog('WriteOuterForInter 异常：' + GlobalOPCItemIn + ' -> ' + outer + ' . ' + e.Message);
  end;
end;
...
begin
  ...
  for i := low(M_Carry) to high(M_Carry) do
    begin
      ...
      M_Carry_Lab[i].parent := M_Carry[i];
...
begin
 ...
 // 发送心跳
      ...
      try
        ...
        on e: Exception do
          formmain.ErrorLog('线程写 PLC 心跳异常： ' + e.Message);
      end;
    // 设备参数
      if (M_DevInit = 0) then
        begin
          try
            ...
            on e: Exception do
              formmain.ErrorLog('线程写 PLC 设备参数异常： ' + e.Message);
          end;
        end;
    ...
    // 读主线位置小车编号 出口编号 出口数量 PLC 工作状态
      try
        ...
        on e: Exception do
          formmain.ErrorLog('线程读 PLC 设备状态异常： ' + e.Message);
      end;
//打印信息
    try
...
//           有打印数据
        ...
        on e: Exception do
          formmain.ErrorLog('线程打印信息入队列异常： ' + e.Message);
      end;
 // 刷新页面
      try
        ...
        on e: Exception do
          formmain.ErrorLog('线程显示设备在线状态异常： ' + e.Message);
      end;
      ...
      try
        ...
        on e: Exception do
          formmain.ErrorLog('线程显示设备主线状态异常： ' + e.Message);
      end;
    end;
 // sleep(50);
end;
...
//    showmessage('4');
    ...
    on e: Exception do
      formmain.ErrorLog('FormClose 异常： ' + e.Message);
  end;
end;
...
//get parameter
  ...
  //报警OPC
    ...
    on e: Exception do
      formmain.ErrorLog('监控主页面参数设置异常： ' + e.Message);
  end;

  //create mainline/in/out below
  try
    c_carWidth := floor(Screen.Width * 2 / M_Cars) - c_gap;
    CreateMainLine;
    CreateIn;
    CreateOut;
    ...
    on e: Exception do
      begin
        formmain.ErrorLog('监控主页面创建页面对象异常： ' + e.Message);
...
//OPC connect
  try
    ...
    on e: Exception do
      formmain.ErrorLog('监控主页面 OPC 通讯设置异常： ' + e.Message);
  end;
...
//创建READPLC 线程
  try
    ...
    on e: Exception do
      begin
        READPLC := nil;
        formmain.ErrorLog('系统错误，无法创建 ReadPLC 线程');
      end;
...
//创建READSCAN 线程
  try
    ...
    on e: Exception do
      begin
        READSCAN := nil;
        formmain.ErrorLog('系统错误，无法创建 ReadSCAN 线程');
      end;
//  raise EError.Create('系统错误，无法创建 ReadPLC 线程')
  end;
