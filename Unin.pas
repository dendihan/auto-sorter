...
//            I_Filename:='123.jpg';
    ...
    //        showmessage(I_Filename);
      ...
      on e: exception do
        begin
          Formmain.ErrorLog('入口队列处理异常： ' + e.Message);
          LockGuid.Release;
        end;
...
//                  StatusBar1.Panels[2].Text := I_TcpRec;
        ...
        on e: exception do
           Formmain.ErrorLog('入口接收数据处理异常： ' + e.Message);
        end;
    end;
//    StatusBar1.Panels[2].Text := I_TcpRec;
...
//        showmessage(c);
    ...
    on e: exception do
      formmain.ErrorLog('入口页面显示异常： ' + e.Message);
  end;
    ...
    on e:exception do
      begin
        ...
        if havelog=0 then
          begin
            havelog:=1;
            Formmain.ErrorLog('Socket 服务器连接异常： ' + e.Message);
          end;
      end;
  end;
end;
  ...
  try
    ...
    on e:exception do
      begin
        ...
        Formmain.ErrorLog('载入图片异常： ' + e.Message);
      end;
...
//close thread
  try
    ...
    on e:exception do
      formmain.ErrorLog('入口操作台关闭异常： ' + e.Message);
  end;
//close comport
...
//  Image1.Align:=alNone;
  Imagresize;
  image1.Height:=trunc(image1.Height*0.95);
...
//  Image1.Align:=alNone;
  Imagresize;
  image1.Height:=trunc(image1.Height*1.0526);
   ...
   // READTCP := TReadTCP.Create(False { true suspended } );
    ...
    on e: exception do
      begin
        READTCP := nil;
        ImgFTP:=nil;
        formmain.ErrorLog('系统异常. 无法创建 ReadTCP 线程: ' + e.Message);
      end;
  end;
